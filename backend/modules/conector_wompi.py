...
async def sincronizar_wompi(credenciales, fecha_inicio, fecha_corte, carpeta_descarga, headless=True):
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto("https://comercios.wompi.co/login", wait_until="networkidle", timeout=30000)
            ...
            await page.goto("https://comercios.wompi.co/transactions", wait_until="networkidle", timeout=30000)
            ...
            await page.click('button:has-text("Aplicar"), button:has-text("Filtrar")')
            await page.wait_for_timeout(2000)

            async with page.expect_download(timeout=60000) as dl_info:
                await page.click('button:has-text("Exportar"), button:has-text("Descargar")')
            download = await dl_info.value
            ruta = os.path.join(carpeta_descarga, f"wompi_{fecha_inicio}_{fecha_corte}.csv")
            await download.save_as(ruta)

            pagos = parsear_reporte_wompi(ruta)
...
def parsear_reporte_wompi(ruta_csv):
    import csv, re
    pagos = []
    with open(ruta_csv, encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if estado not in ("APROBADA", "APPROVED"):
                continue
            valor_str = row.get("Monto", row.get("Valor", row.get("Total", "0")))
            valor_neto = float(re.sub(r"[^\d.]", "", valor_str or "0") or 0) / 100
            fecha_str = row.get("Fecha", row.get("Fecha de creacion", ""))
            try:
            ...
            pagos.append({
                ...
                "metodo_pago": row.get("Medio de pago", row.get("Franquicia", "default")).lower(),
