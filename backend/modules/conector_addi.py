...
async def sincronizar_addi(credenciales, fecha_inicio, fecha_corte, carpeta_descarga, headless=True):
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto("https://app.addi.com/login", wait_until="networkidle", timeout=30000)
            await page.fill('input[type="email"], input[name="email"]', credenciales["email"])
            await page.fill('input[type="password"], input[name="password"]', credenciales["password"])
            await page.click('button[type="submit"]')
            await page.wait_for_url("**addi.com/**", timeout=20000)

            await page.goto("https://app.addi.com/payments", wait_until="networkidle", timeout=30000)
            ...
            ruta = os.path.join(carpeta_descarga, f"addi_{fecha_inicio}_{fecha_corte}.csv")
            await download.save_as(ruta)

            pagos = parsear_reporte_addi(ruta)
...
def parsear_reporte_addi(ruta_csv):
    import csv, re
    pagos = []
    with open(ruta_csv, encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            pagos.append({
                ...
                "referencia": row.get("ID orden", row.get("Referencia", "")),
                "metodo_pago": "addi",
